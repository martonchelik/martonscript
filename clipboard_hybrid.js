    ...
    if (inp) {
        ...
        function createListItem(item, val, index, inputValue, i) {
            ...
            b.addEventListener("click", function (e) {
                ...
                if (inp.value === "В чеке инстант"){
                    let checkMessage = userUrl + ' ставки в instant games под низкий коэффициент, проверьте, пожалуйста'
                    navigator.clipboard.writeText(checkMessage).then(() =>{console.log(checkMessage)});
                    inp.value = 'В чеке'
                }
                if (inp.value === "В чеке купон"){
                    let checkMessage = userUrl + ' отыгран купон, проверьте, пожалуйста'
                    navigator.clipboard.writeText(checkMessage).then(() =>{console.log(checkMessage)});
                    inp.value = 'В чеке'
                }
                if (inp.value === "В чеке рулетка 2-3"){
                    let checkMessage = userUrl + ' рулетка х2-х3, проверьте, пожалуйста'
                    navigator.clipboard.writeText(checkMessage).then(() =>{console.log(checkMessage)});
                    inp.value = 'В чеке'
                }
                if (inp.value === "В чеке рулетка вб"){
                    let checkMessage = userUrl + ' рулетка, ставки на весь баланс, проверьте, пожалуйста'
                    navigator.clipboard.writeText(checkMessage).then(() =>{console.log(checkMessage)});
                    inp.value = 'В чеке'
                }
                if (inp.value === "В чеке рулетка 2-3 вб"){
                    let checkMessage = userUrl + ' рулетка х2-х3, ставки на весь баланс, проверьте, пожалуйста'
                    navigator.clipboard.writeText(checkMessage).then(() =>{console.log(checkMessage)});
                    inp.value = 'В чеке'
                }
                if (inp.value === "В чеке красная бонусная"){
                    let checkMessage = userUrl + ' красная выигрышная бонусная ставка, проверьте, пожалуйста'
                    navigator.clipboard.writeText(checkMessage).then(() =>{console.log(checkMessage)});
                    inp.value = 'В чеке'
                }
                if (inp.value === "В чеке крупный выигрыш"){
                    let checkMessage = userUrl + ' крупный выигрыш, проверьте, пожалуйста'
                    navigator.clipboard.writeText(checkMessage).then(() =>{console.log(checkMessage)});
                    inp.value = 'В чеке'
                }
                ...
                if (inp.value === "В чеке блэкджек"){
                    let checkMessage = userUrl + ' blackjack, проверьте, пожалуйста'
                    navigator.clipboard.writeText(checkMessage).then(() =>{console.log(checkMessage)});
                    inp.value = 'В чеке'
                }
...
document.addEventListener('readystatechange', () => {
    if (document.readyState === 'complete') {
        chrome.storage.local.get('checkboxStates', function(result) {
            if (result.checkboxStates && result.checkboxStates.clipboardSet && result.checkboxStates.profileSet && window.location.href.split('/')[4]!="") {
                setupAutocomplete(result.checkboxStates.documentCommentTemplatesSet);
            }else if(window.location.href.split('/')[4]==""){
                docPanelScroll();
            }
        });
        chrome.storage.local.get('checkboxStates', function(result) {
...
/*    setupClickCancel();*/
    ...
    function findAllButtons() {
        waitForClass('a.button.toggle-approve.comment-document', (buttons) => {
            buttons.forEach((button) => {
                button.addEventListener('click', () => {
                    currentUIID();
        ...
        waitForClass('a.button.toggle-approve.approve-document', (buttons) => {
            buttons.forEach((button) => {
                button.addEventListener('click', () => {
        ...
        waitForClass('a.button.toggle-approve.update-document', (buttons) => {
            buttons.forEach((button) => {
                button.addEventListener('click', () => {
        ...
        waitForClass('a.button.toggle-approve.unapprove-document', (buttons) => {
            buttons.forEach((button) => {
                button.addEventListener('click', () => {
                    cancelDocComments();
                })
    ...
    function autocompleteDocComments(documentCommentTemplatesSet) {
        if (documentCommentTemplatesSet){
            waitForElement('comment_document_sidebar_section', (element) => {
                const docComment = element.getElementsByTagName('textarea')[0];
